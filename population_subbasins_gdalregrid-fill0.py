...
regclip = {'Brahmaputra':'brahmaputra1','Meghna':'Meghna','Ganges':'ganges_subset','Bangladesh':'','Bangladesh-north':'BangladeshBox-clipsouth','Meghna-north':'Meghna-clipsouth'}
...
for reg,fpart in regclip.items():
	tmpfile = os.path.join(pop_dir,'tmp.tif')
	...
	if not os.path.exists(pop_clip):
		if fpart != '':
			...
			gdal_cmd = ['gdalwarp','-of','GTiff','-tr','0.0025','-0.0025','-te','87.627916667','21.131250000','92.737916667','26.681250000','-cutline',fclip,'-dstnodata','nan',input_pop,tmpfile]
			print(' '.join(gdal_cmd))
			subprocess.call(gdal_cmd)
		else:
			# Reproject but dont clip
			gdal_cmd = ['gdalwarp','-of','GTiff','-tr','0.0025','-0.0025','-te','87.627916667','21.131250000','92.737916667','26.681250000','-dstnodata','nan',input_pop,tmpfile]
			print(' '.join(gdal_cmd))
			subprocess.call(gdal_cmd)

		# Now fill nans with 0:
		gdal_cmd2 = ['gdal_calc.py','--calc',"where(isnan(A),0,A)",'--format','GTiff','--type','Float32','-A',tmpfile,'--A_band','1','--co','COMPRESS=DEFLATE','--outfile',pop_clip]

		print(' '.join(gdal_cmd2))
		subprocess.call(gdal_cmd2)
		os.remove(tmpfile)
